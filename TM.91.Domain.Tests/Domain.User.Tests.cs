...
public class TestData
{
    ...
    public UserState UserAdminWithData { get; set; } = new()
        {
...
public class UserTests
{
    ...
    public void TestUserNameUpdate()
    {
        ...
        var updateNameR_failure = UserCommmand.UpdateUserName_R("", testUser.UserAdminWithData);

        Assert.True(updateNameR_failure.IsFailure);
        Assert.Equal(updateNameR_failure.Error, new Error("Error.InvalidNameString", ""));

        var updateNameR_success = UserCommmand.UpdateUserName_R("Test2", testUser.UserAdminWithData);
    ...
    public void TestUserEmailUpdate()
    {
        ...
        var updateEmailR_failure = UserCommmand.UpdateUserEmail_R("", testUser.UserAdminWithData);

        Assert.True(updateEmailR_failure.IsFailure);
        Assert.Equal(updateEmailR_failure.Error, new Error("Error.InvalidEmailString", ""));

        var updateEmailR_success = UserCommmand.UpdateUserEmail_R("test2@test.co.za", testUser.UserAdminWithData);
    ...
    public void TestUserPasswordUpdate()
    {
        ...
        var updatePasswordR_failure = UserCommmand.UpdateUserPassword_R("", "NewSecureP@ss1", testUser.UserAdminWithData);
        ...
        var updatePasswordR_success = UserCommmand.UpdateUserPassword_R("Test1234", "NewSecureP@ss1", testUser.UserAdminWithData);
